...
class OrderPartService:
    def create_order_part(
        self, order_id: OrderID, part_id: PartID, qty: int, price: float | None = None
    ) -> OrderPart:
        return OrderPart(
            id=None, uuid=uuid.uuid4(), order_id=order_id, part_id=part_id, qty=qty, price=price
        )

    def update_order_part(
        self, order_part: OrderPart, qty: int | None = None, price: float | None = None
    ) -> OrderPart:
        if qty is not None:
            order_part.qty = qty
